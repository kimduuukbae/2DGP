...
class Boss_Battle_Sprite:
    ...
    def draw(self):
        ...
        self.hero.draw(600 + self.shake_float, 100 + self.shake_float + self.camera_y // 10)
        self.font.draw(450 + self.shake_float, 160 + self.shake_float + self.camera_y // 10, "전사", (255, 255, 255))
        if self.hero.shield == 0:
            self.font.draw(555 + self.shake_float, 102 + self.shake_float + self.camera_y // 10,
                           str(self.hero.get_hp()) + ' / ' + str(self.hero.get_maxhp()), (255, 255, 255))
        else:
            self.font.draw(535 + self.shake_float, 102 + self.shake_float + self.camera_y // 10,
                           str(self.hero.get_hp()) + ' / ' + str(self.hero.get_maxhp()) + ' + ' + str(self.hero.shield),
                           (255, 255, 150))

        self.monster.draw(1750 + self.shake_float, 800 + self.shake_float + self.camera_y // 10)
        self.font.draw(1600 + self.shake_float, 860 + self.shake_float + self.camera_y // 10, self.monster.name,
                       (255, 255, 255))
        self.font.draw(1705 + self.shake_float, 802 + self.shake_float + self.camera_y // 10,
        ...
        Battle_state_sprite.condition_image.draw(600 + self.shake_float, 45 + self.shake_float + self.camera_y // 10)
        Battle_state_sprite.condition_image.draw(1750 + self.shake_float, 745 + self.shake_float + self.camera_y // 10)
        ...
        for i in range(len(self.hero.status_condition.get_condition_list())):
            ...
            self.font.draw(470 + (i * 150) + self.shake_float, 46 + self.shake_float + self.camera_y // 10 ,
                           condition_name + "  " + str(get_condition_count), (139, 0, 255))

        if self.victory_flag == 3:
            Boss_Battle_Sprite.banner.draw()
