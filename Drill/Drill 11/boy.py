...
class JumpState:
    ...
    def do(boy):
        boy.frame = (boy.frame + FRAMES_PER_ACTION * ACTION_PER_TIME * game_framework.frame_time) % 8
        boy.y += boy.dirY * boy.velY * game_framework.frame_time
        boy.x += boy.velocity * game_framework.frame_time
        boy.yCount += 1
        if boy.yCount > 300 and boy.dirY == 1:
            boy.yCount = 0
            boy.dirY = -boy.dirY
            boy.type = 0
...
next_state_table = {
    ...
    JumpState : {LEFT_DOWN: JumpState, RIGHT_DOWN: JumpState, LEFT_UP: JumpState, RIGHT_UP: JumpState, SPACE: JumpState}
}
...
class Boy:
    ...
    def stop(self):
        if self.cur_state == JumpState and self.dirY == -1:
            if self.velocity != 0:
